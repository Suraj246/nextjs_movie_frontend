...
import { deleteMovie, fetchMovies } from '@/app/redux/slices/movieSlice'
...
import { fetchDataByType } from '@/app/redux/slices/movieTypeSlice'
import GenreFilter from './GenreFilter'


const MovieType = () => {
    const dispatch = useDispatch<AppDispatch>()
    const search: string | null = useSearchParams().get('type')
    const [user, setUser] = useState<string>('')

    const inputValue = useSelector(selectInputValue)

    useEffect(() => {
        dispatch(fetchDataByType(search))
    }, [dispatch, search])


    const movieList = useSelector((state: RootState) => state.movieTypeData)
    const { movietypeData, status, error } = movieList
